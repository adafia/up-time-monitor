...
// Append a string to a file and create the file if doesnt exist
        ...
        if (!err && data && data.length > 0) {
            ...
            data.forEach((fileName) => {
               // Add the .log files
                ...
                // Optionall add the compressed files i.e add on the .gz files
            ...
            zlib.gzip(inputString, (err, buffer) => {
                if (!err && buffer) {
                    ...
                    fs.open(lib.baseDir + desFile, 'wx', (err, fileDescriptor) => {
                        if (!err && fileDescriptor) {
                           // Write to the destinartion file
...
lib.decompress = (fileId, callback) => {
    ...
    readFile(lib.baseDir + fileName, 'utf-8', (err, str) => {
        if (!err && str) {
            // Decompress the data
