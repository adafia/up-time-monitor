                                    ...
                                    _data.update('users', userPhone, userData, (err) => {
                                        if (!err) {
                                           // Return the data about the new check
                                            ...
                                            callback(500, {'Error':'Could not update the user object with the new check'})
                                       }
                                    });
                                } else {
    ...
    if (id) {
        ...
        _data.read('checks', id, (err, checkData) => {
            if (!err && checkData) {
                // Get the token from the headers
                const token = typeof (data.headers.token) == 'string' ? data.headers.token : false;
                // verify that the given token from headers is valid and belongs to the user doing the check
...
handlers._checks.put = (data, callback) => {
    ...
    if (id) {
        // Check to make sure one or more of the optional fields have been set.
